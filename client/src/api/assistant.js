...
export const fetchAllAssistants = async (
  ...
  const response = await axiosSecureInstance.get(
    SUPER_ADMIN_GET_ALL_ASSISTANTS_SLUG(page, limit, orgAssistantSearchQuery)
  );
...
export const fetchAssistantsCreatedByUser = async (
  page,
  userId,
...
export const updateAssistantWithDetailsAndFiles = async (
  assistantId,
  payload
) => {
  const response = await axiosSecureInstance.patch(
...
export const updateAssistantAccessForTeam = async (teamId, payload) => {
  const response = await axiosSecureInstance.patch(
    UPDATE_ASSISTANT_ACCESS_FOR_TEAM_API(teamId),
    payload
  );
